...
class ConfigParser:
    ...
    def _validate_globals(self):
        ...
        missing_optional_keys = [param for param in optional if param not in globals_config]
        ...
        if globals_config["loss_function"] not in ("cross_entropy", "mse"):
            raise ValueError(f"In config GLOBALS: loss_function must be cross_entropy or mse")
        ...
        if unexpected_keys:
            raise ValueError(f"Unexpected key(s) in config GLOBALS: {', '.join(unexpected_keys)}")
    ...
    def _validate_input_layer(self):
        ...
        required_keys_with_none_values = [param for param in required if param in input_layer[0] and input_layer[0][param] is None]
        if required_keys_with_none_values:
            raise ValueError(f"Required key(s) is set to None: {required_keys_with_none_values}")
        
        unexpected_keys = [key for key in input_layer[0] if key not in required]
        if unexpected_keys:
    ...
    def _validate_hidden_layers(self):
        ...
        hidden_layers = [layer for layer in layers_config if layer["layer_type"] == "hidden"]
        if not hidden_layers:
            return

        required = ["layer_type", "neurons", "activation_function", "weight_range", "bias_range"]
        optional = ["learning_rate"]

        for layer in hidden_layers:
            missing_required_keys = [param for param in required if param not in layer]
            missing_optional_keys = [param for param in optional if param not in layer]
            if missing_required_keys or missing_optional_keys:
                raise ValueError(f"Missing key(s) in hidden layer: {', '.join(missing_required_keys + missing_optional_keys)}")

            required_keys_with_none_values = [param for param in required if layer[param] is None]
            if required_keys_with_none_values:
                raise ValueError(f"Required key(s) with None value in hidden layer: {required_keys_with_none_values}")
            
            if layer["activation_function"] not in ("relu", "identity"):
                raise ValueError(f"Invalid activation function for hidden layer: {layer['activation_function']}")
    ...
    def _validate_output_layer(self):
        ...
        if not output_layer or len(output_layer) > 1:
            raise ValueError("Either no output layer or more than one output layer in config")
